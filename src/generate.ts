...
class Generator implements ParsedOpenApi{
  ...
  private getType(
    ...
    if ("$ref" in obj) {
      ...
      this.namedTypes[typeName] = {impl: typeImpl, description: schema.description}
...
export async function generate(
  ...
  if (options.types){
    output += Object.entries(generator.namedTypes)
      .map(([name, typeImpl]) => formatTypeDeclaration(name, typeImpl.impl, options.withDoc ? typeImpl.description: ''))
      .join("\n\n")
    output += "\n\n"
  }
  if (options.request){
  ...
  if (output){
    output = "// This file is generated by [oats][0] and should not be edited by hand.\n//\n// [0]: https://github.com/influxdata/oats\n\n"+output
  }
